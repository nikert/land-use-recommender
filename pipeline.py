"""
Байесовский KDE-классификатор потенциала ВРИ (geopandas + rasterio).

Запуск:  python pipeline.py /path/to/data/
В папке ожидаются: egrn, poi_point, poi_polygon, roads, zouit, dem,
boundary_1..3 (необязательные слои пропускаются).

Результаты пишутся в /path/to/data/results_YYYY-MM-DD_HH-MM-SS/.
"""
...
# для оценки территорий — равномерный приор
...
def assess_pt_pe(sl, lc, pd, rd, zv):
    ...
    # min-max в пределах участка: лучший ВРИ → 1, худший → 0
    def _minmax(d):
